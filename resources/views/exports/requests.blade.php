...
<head>
    ...
    <title>Laporan Permintaan - SMKN 11 Bandung - {{ Date::now()->format('F Y') }}</title>
    <script>
...
<body>
    ...
    <h4 style="text-align: center; margin-top: 10px;">LAPORAN DATA PERMINTAAN</h4>
    ...
    <p>Disampaikan dengan hormat, kami dari Unit Kerja Sarana SMKN 11 Bandung sampaikan laporan data permintaan dari unit
        untuk periode
        {{ str(date_format($date, 'd F Y')) ?? Date::now()->format('d F Y') }}.</p>
    <p>Dengan ini kami sampaikan laporan data permintaan sebagai berikut:</p>
    ...
    <table class="table">
        ...
        <tbody>
            @foreach ($requests as $index => $request)
                @php
                    $username = trim(explode('@', $request->user_name)[0]);
                @endphp
                <tr>
                    <td style="font-size: 13px; text-align: center">{{ $index + 1 }}</td>
                    <td style="font-size: 13px; text-align: center">{{ strtoupper($username) }}</td>
                    <td style="font-size: 13px; text-align: center">{{ $request->status }}</td>
                    <td style="font-size: 13px; text-align: center">{{ $request->item_name }}</td>
                    <td style="font-size: 13px; text-align: center">{{ $request->qty_item }}</td>
                    <td style="font-size: 13px; text-align: center">{{ $request->qty_approved }}</td>
                    <td style="font-size: 13px">{{ $request->regarding }}</td>
                    <td style="font-size: 13px">{{ $request->created_at->format('d F Y') }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
