<x-form id="requestForm" wire:submit.prevent="store" class="space-y-2" autocomplete="off" no-separator>
    <div class="grid w-full grid-cols-1 gap-4">
        <div class="flex w-full gap-5 flex-nowrap">
            <div class="w-1/2">
                <x-textarea label="Perihal" wire:model="regarding" placeholder="Type here ..." rows="3"
                    inline />
            </div>
            <div class="w-1/2">
                <x-textarea label="Sifat" wire:model="characteristic" placeholder="Type here ..."
                    rows="3" inline />
            </div>
        </div>
        <!-- Dynamic Inputs Loop -->
    ...
    <x-slot:actions>
        <x-button label="Submit" icon="c-paper-airplane"
            class="w-full text-white btn-primary md:w-auto lg:w-auto" type="submit" spinner="store" />
    </x-slot:actions>
</x-form>
