<x-card class="mx-10">
    <x-header wire:model.live.debounce="search" title="My Submission's" class="px-3 pt-3" size="text-4xl"
    ...
    <x-table :headers="$headers" :rows="$submissions" :sort-by="$sortBy" link="/submissions/{id}"
        ...
        @scope('actions', $submission)
            <x-button icon="o-information-circle" link="{{ route('submissions.show', $submission->id) }}"
