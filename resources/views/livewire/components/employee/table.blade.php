<x-card>
    <x-header wire:model.live.debounce="search" title="Pegawai" class="px-3 pt-3" size="text-3xl" subtitle="Employee Table"
        ...
        <x-slot:actions>
            ...
            <x-button icon-right="m-plus" label="Add" wire:click="createEmployeesModal"
                class="text-white bg-dark dark:bg-slate-100 hover:bg-dark hover:opacity-90 dark:text-black" responsive
                aria-label="create item" />
        </x-slot:actions>
    </x-header>

    <x-table :headers="$headers" :rows="$employees" :sort-by="$sortBy" link="/employees/{id}"
    ...
    <x-modal wire:model="createEmployees" class="backdrop-blur" box-class="w-full lg:min-w-[800px] md:min-w-[800px]">
        ...
        <x-card>
            <x-form id="suppliersForm" wire:submit.prevent="store" class="space-y-4" autocomplete="off" no-separator>
                <x-input wire:model="newEmployee.id" id="id" label="NIP" inline />
                <x-input wire:model="newEmployee.name" id="name" label="Name" inline />
