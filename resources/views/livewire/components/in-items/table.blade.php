<x-card>
    ...
    <x-table :headers="$headers" :rows="$itemsIn" :sort-by="$sortBy" link="/in-items/{id}"
        ...
        @scope('actions', $itemsIn)
            <x-button icon="o-trash" wire:click="delete({{ $itemsIn->id }})" class="btn-sm btn-ghost dark:text-slate-300 btn-outline" 
    ...
    <x-drawer title="Filter" wire:model="drawerIsOpen" class="w-1/2 lg:w-1/3" right separator with-close-button
        ...
        <x-form wire:submit="itemsIn" no-separator>
            ...
            <x-slot:actions>
                ...
                <x-button label="Save" class="text-white btn-primary" icon="c-paper-airplane" type="submit" spinner="itemsIn" />
            </x-slot:actions>
        </x-form>
    </x-drawer>
</x-card>
